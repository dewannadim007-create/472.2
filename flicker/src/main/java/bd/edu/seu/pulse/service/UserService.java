...
public class UserService {
                ...
                for (String interest : interests) {
                    boolean found = writer.getInterests().stream()
                            .anyMatch(writerInterest -> {
                                if (writerInterest == null)
                                    return false;

                                String writerInterestLower = writerInterest.toLowerCase();
                                String interestLower = interest.toLowerCase();

                                if (writerInterestLower.equals(interestLower))
                                    return true;

                                if (writerInterestLower.contains(interestLower))
                                    return true;

                                if (interestLower.contains(writerInterestLower))
                                    return true;
    ...
    public String updateProfilePhoto(String userId, org.springframework.web.multipart.MultipartFile photoFile) {
        try {
            User user = userRepository.findById(userId).orElse(null);
            if (user == null) {
                return "User not found";
    ...
    public String removeProfilePhoto(String userId) {
        User user = userRepository.findById(userId).orElse(null);
        if (user == null) {
            return "User not found";
        }
